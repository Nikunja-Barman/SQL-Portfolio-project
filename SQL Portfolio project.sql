...
Select SUM(new_cases) as total_cases, SUM(cast(new_deaths as int)) as total_deaths, SUM(cast(new_deaths as decimal))/SUM(New_Cases)*100 as DeathPercentage
From Covid_deaths
...
Max((total_cases/cast(population as decimal)))*100 as PercentPopulationInfected
From Covid_Deaths
--Where location like '%states%'
Group by Location, Population
